...
def handle_minecraft_trigger():
    ...
    if not my_manager_info and not any_manager_info:
        ...
        manager_ip = local_ip
        token = ""
        print(f"--- [ELEIÇÃO LÍDER GLOBAL] {node_name} será o Manager Primário! ---")
        consul_manager.save_cluster_state(REGION, {"manager_ip": manager_ip, "manager_token": "", "worker_token": ""})
        ...
        token = any_manager_info["manager_token"]  # Une-se usando o token de manager!
        print(f"--- [CONEXÃO INTER-REGIÃO] {node_name} liga-se como Manager Secundário ao cluster em {manager_ip} ---")
        consul_manager.save_cluster_state(REGION, {"manager_ip": local_ip, "manager_token": token, "worker_token": any_manager_info["worker_token"]})
        
    else:
        # A minha região já tem manager próprio. Este entra como worker local
        role = "worker"
        manager_ip = my_manager_info["manager_ip"]
        token = my_manager_info["worker_token"]
        print(f"--- [WORKER LOCAL] {node_name} junta-se ao Manager da região {REGION} ---")
